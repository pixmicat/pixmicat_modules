...
程式<select name="Papplet"><option value="0">PaintBBS</option><option value="1">しぃペインター</option><option value="2" selected="selected">しぃペインターPro</option></select>
...
<br /><a href="'.$this->THISPAGE.'&amp;action=post'.($isReply ? '&amp;resto='.$isReply : '').'">使用先前繪圖</a>
	...
	public function autoHookThreadPost(&$arrLabels, $post, $isReply){
		...
		$pchType = '';
		if (file_exists($pchBase.'.pch')) { 
			$pchFile = $post['tim'].'.pch'; 
		} elseif (file_exists($pchBase.'.spch')) { 
			$pchFile = $post['tim'].'.spch';
			$pchType = '&amp;type=spch';
		} else {
			return;
		}
	...
	public function autoHookRegistBeforeCommit(&$name, &$email, &$sub, &$com, &$category, &$age, $dest, $isReply, $imgWH, &$status){
		...
		if(isset($_POST['PaintSend'])){ // 繪圖模式送來的儲存
			...
			if (file_exists($this->TMPFolder.$pchOldfile.'.pch')) { 
				$pchOldfile = $this->TMPFolder.$pchOldfile.'.pch';
				$pchNewfile .= '.pch';
			} elseif (file_exists($this->TMPFolder.$pchOldfile.'.spch')) {
				$pchOldfile = $this->TMPFolder.$pchOldfile.'.spch';
				$pchNewfile .= '.spch';
			} else { 
				return;
			}

			copy($pchOldfile, $pchNewfile);
			unlink($pchOldfile);
		}
	}
...
'.$imgList.'
	...
	private function Action_ViewPCH(){
		...
		$pchUrl = $FileIO->getImageURL(basename($pchName)); // TODO: Only support Local
...
<param name="pch_file" value="'.$pchUrl.'" />
...
<p>-<a href="'.$pchUrl.'">Download</a>-<br />('.$pchSize.' bytes)</p>
</div>
<hr />';
		foot($dat);
		echo $dat;
