<?php
class mod_vipquality extends ModuleHelper {
	private $usec;
	private $sec;
	private $omikuji = array('大吉','中吉','吉','小吉','末吉','凶','大凶','豚','ぴょん吉','だん吉','神','女神');
	
	public function __construct($PMS) {
		parent::__construct($PMS);
		list($this->usec, $this->sec) = explode(" ", microtime());
	    mt_srand(intval($this->usec*1000 + $this->sec));
	}
	...
	public function autoHookRegistBeforeCommit(&$name, &$email, &$sub, &$com, &$category, &$age, $dest, $isReply, $imgWH, &$status){
		...
		if(strpos($name,'fusianasan')!==false) $name=str_replace('fusianasan','<span class="nor">'. getREMOTE_ADDR().'</span>',$name);
		if(strpos($name,'mokorikomo')!==false) $name=str_replace('mokorikomo','<span class="nor">'. getREMOTE_ADDR().'</span>',$name);
	}

	private function _uptime(&$name) {
		$loadavg='';
	...
	private function _omikuji(&$name) {
		...
		$name=str_replace('!omikuji','<span class="nor">【'.$this->omikuji[$omi_v].'】</span>',$name);
	}
}
