...
class mod_opentag{
	...
	function getModuleName(){
		return 'mod_opentag : Pixmicat! Open Tagging Module (Alpha)';
	}

	/* Get the module version infomation */
	function getModuleVersionInfo(){
		return 'Pixmicat! Open Tagging Module (Alpha) v070925';
	}
	...
	function ModulePage(){
		...
		if(!isset($_POST['tag'])) {
			...
			$pte_vals = array('{$TITLE}'=>TITLE,'{$RESTO}'=>''); $dat='';
			$dat .= $PTE->ParseBlock('HEADER',$pte_vals);
			$dat .= "</head><body id='shoutbox_main'>";
			$dat.='<form action="'.$this->mypage.'&amp;no='.$_GET['no'].'" method="POST"><input type="text" name="tag" value="'.substr(str_replace('&#44;', ',', $post[0]['category']),1,-1).'" size="28"/><input type="submit" name="submit" value="Tag!"/></form>';
			echo $dat."</body></html>";
		} else {
