		...
		if(preg_match('/(<a.*">)?(.*)<span class\="nor">'._T('trip_pre').'(.*?)<\/span>(<\/a>)?/', $arrLabels['{$NAME}'], $matches)) {
			if($matches[3]) { // Trip found
				if($matches[2]) { // has name
					...
					$arrLabels['{$NAME}']='<span class="nor"><a href="'.$this->myPage.'&amp;action=write&amp;t='.$matches[3].'" style="text-decoration: overline underline" title="PM">'._T('trip_pre').'</a>'.$matches[1].$matches[3].($matches[1]?'</a>':'')."</span>";
				}
			}
		}
	}
