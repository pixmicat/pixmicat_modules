	...
	function autoHookThreadPost(&$arrLabels, $post, $isReply){
		...
		if(ip2long($iphost)!==false) {
			...
			if(preg_match('/([\w\-]+)\.([\w\-]+)$/',$iphost,$parts)) {
				...
				// asretelecom/cgocable/spcsdns/indiana.edu/metrocast/twtelecom/frontiernet/onecommunications/dslextreme/slicehost/as29550/clearwire-wmx IP hack
				if($parts[1] == 'hinet' || $parts[1] == 'teksavvy' || $parts[1] == 'qwest'
				 ...
				 || $parts[1] == 'frontiernet' || $parts[1] == 'onecommunications' || $parts[1] == 'dslextreme' || $parts[1] == 'slicehost' || $parts[1] == 'as29550' || $parts[1] == 'clearwire-wmx') {
				...
				// netvigator/bbtec/HKBN IP hack
					...
					elseif($parts[1] == 'netvigator') {
						...
						$parthost = '*.'.$parts[0];
				// pldt/quadranet/totbb/plus/ono/edpnet IP hack
				} elseif($parts[1] == 'pldt' || $parts[1] == 'quadranet' || $parts[1] == 'totbb' || $parts[1] == 'plus' || $parts[1] == 'ono' || $parts[1] == 'edpnet') {
					if(preg_match('/^(\d+\.\d+)/',$iphost,$ipparts))
						$parthost = $ipparts[1].'.*.'.$parts[0];
					else
						$parthost = '*.'.$parts[0];
				} elseif($parts[1] == 'bjtelecom' || $parts[1] == 'cndata') { // bjtelecom/cndata IP hack
					...
					if(preg_match('/^[a-z\-]*\d+\-\d+\-(\d+\-\d+)\-\d+\-\d+/',$iphost,$ipparts))
						...
						$parthost = '*.'.$parts[0];
				} elseif($parts[1] == 'comunitel' || $parts[1] == 'ukrtel' || $parts[1] == 'vtr') { // comunitel/ukrtel/vtr IP hack
						...
						foreach($cctld as $tld) {
							if($lastpart == $tld) {
								...
								if($isgtld) {
									...
									// adsl24/btvm/netspace.net.au/connections.net.nz/orcon.net.nz IP hack
									if($parts[1] == 'kbronet' || $parts[1] == 'seed' || $parts[1] == 'so-net'
									 ...
									 || $parts[1] == 'netspace' || $parts[1] == 'connections' || $parts[1] == 'orcon') {
										if(preg_match('/^(\d+\-\d+)/',$iphost,$ipparts))
										...
										elseif($parts[1] == 'seed' && preg_match('/^\w+\-(\d+\-\d+)-\d+/',$iphost,$ipparts)) // seed h* IP hack
											...
											$parthost = '*.'.$parts[0];
									// i-cable/singnet/optusnet/plala/rosenet/bethere/asianet/home.ne.jp/hidatakayama/apol/pikara/bigpond/netspace/orange.co.il/
									// callplus/prod-infinitum/xnet/unwired/e-mobile/gol.ne.jp/ksu.edu.tw/monash.edu.au/sinica.edu.tw/cjcu.edu.tw/asianet/tic.net.tw/dialog.net.pl IP hack
									 ...
									 || $parts[1] == 'sinica' || $parts[1] == 'cjcu' || $parts[1] == 'asianet' || $parts[1] == 'tic' || $parts[1] == 'dialog') {
									...
									// corbina/j-cnet/numericable/telekom.at/tele2.se/kou/bbiq/inode/superkabel/novis.pt IP hack
									} elseif($parts[2] == 'corbina' || $parts[2] == 'j-cnet' || $parts[2] == 'numericable'
									|| $parts[2] == 'telekom' || $parts[2] == 'tele2' || $parts[2] == 'kou' || $parts[2] == 'bbiq'
									|| $parts[2] == 'inode' || $parts[2] == 'superkabel' || $parts[2] == 'novis') {
										if(preg_match('/^(\d+\-\d+)/',$iphost,$ipparts))
									...
									// mnet-online/upcbroadband/scarlet.be/ownit/itscom/sk.ca/swipnet.se IP hack
									...
									|| $parts[2] == 'itscom' || $parts[2] == 'sk' || $parts[2] == 'swipnet') {
										if(preg_match('/^[a-z]*-?(\d+\-\d+)/',$iphost,$ipparts))
											$parthost = $ipparts[1].'-*.'.$parts[2].'.'.$parts[3];
										...
										if(preg_match('/^\w+\-([0-9a-f]{2})([0-9a-f]{2})[0-9a-f]{2}[0-9a-f]{2}\./',$iphost,$ipparts))
											...
											$parthost = '*.'.$parts[2].'.'.$parts[3];
									} elseif($parts[2] == 'cybercity' || $parts[2] == 'ziggozakelijk') { // cybercity.dk/ziggozakelijk.nl IP hack
										...
										if(preg_match('/^\w+\-\w+\-(\d+-\d+)-\d+-\d+/',$iphost,$ipparts))
											...
											$parthost = '*.'.$parts[2].'.'.$parts[3];
									// gavle/t-ipconnect/telenet/direct-adsl IP hack
